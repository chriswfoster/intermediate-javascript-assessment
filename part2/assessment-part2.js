...
function noWeakLink() {
  return $http({
    ...
    .then(response => (thirdUser = response.data[3]))
}
...
return function closureFunct(liabilities){
...
var frodoobj = {hunger: startingHungerValue,
danger: startingDangerValue }
return {
  dinnerOverFire :function(){
    frodoobj.hunger -= 25
    frodoobj.danger +=40
    if(frodoobj.danger > 100){frodoobj.danger = 100}
    if(frodoobj.hunger < 0){frodoobj.hunger = 0}
    return frodoobj
      },
      hidingInBush: function(){
        frodoobj.hunger +=35;
        frodoobj.danger -=20
        if(frodoobj.hunger > 100){frodoobj.hunger = 100}
        if(frodoobj.danger < 0){frodoobj.danger = 0}
        return frodoobj
      }
